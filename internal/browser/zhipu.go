...
func (z *ZhipuClient) handleCaptcha(ctx context.Context) (bool, string) {
	...
	// 策略3：等待用户手动完成验证（最多等待60秒）
	log.Println("========================================")
	log.Println("🔐 需要手动完成人机验证！")
	log.Println("请在浏览器中完成验证操作...")
	log.Println("等待验证通过（最多60秒）...")
	log.Println("========================================")

	maxCaptchaWait := 60 * time.Second
	...
	for time.Since(captchaStart) < maxCaptchaWait {
		...
		log.Printf("等待验证通过...（已等待: %v）", time.Since(captchaStart))
	}

	log.Println("❌ 人机验证等待超时")
	return false, "人机验证等待超时（60秒）"
}
...
func (z *ZhipuClient) Ask(question string) (*AskResult, error) {
	...
	anomalyType, keyword, context := z.detectPageAnomaly(ctx)
	if anomalyType == "input_error" {
		log.Printf("⚠️ 检测到输入超限提示: 关键词=%s, 上下文=%s", keyword, context)
		close(streamChan)
		result.IsBot = false
		result.DetectInfo = fmt.Sprintf("输入超限: %s", context)
		result.Answer = ""
		result.Thinking = ""
		return result, fmt.Errorf("输入内容超限: %s", context)
	}
	if anomalyType == "captcha" {
		log.Printf("⚠️ 检测到人机验证: 关键词=%s, 上下文=%s", keyword, context)
		// 尝试自动处理验证
	...
	for time.Since(startTime) < maxWaitTime {
		...
		if err := json.Unmarshal([]byte(resultStr), &result); err == nil {
			...
			hasNonThinkingReply = result.HasNonThinkingReply
			thinkingLength = result.ThinkingLength
		}
