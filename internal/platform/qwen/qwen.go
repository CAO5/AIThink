// Package qwen 千问（通义千问）平台适配器
// 实现PlatformClient接口，将千问平台的功能适配到统一平台架构
package qwen
...
// 千问网站URL常量
const (
	qwenLoginURL = "https://tongyi.aliyun.com/"
	qwenChatURL  = "https://tongyi.aliyun.com/qianwen/"
)

// QwenClient 千问（通义千问）平台客户端
// 嵌入BrowserSession以复用浏览器会话管理能力
type QwenClient struct {
	session *browser.BrowserSession
}

// NewQwenClient 创建千问客户端
func NewQwenClient(session *browser.BrowserSession) *QwenClient {
	return &QwenClient{session: session}
}
...
func init() {
	platform.GetRegistry().Register(
		...
		func(session *browser.BrowserSession) platform.PlatformClient {
			return NewQwenClient(session)
		},
		&platform.PlatformConfig{
			Platform: models.PlatformQwen,
...
func (q *QwenClient) GetPlatformName() string {
	return "qwen"
}

// GetLoginURL 获取平台登录页面URL
func (q *QwenClient) GetLoginURL() string {
	return qwenLoginURL
}

// GetChatURL 获取平台聊天页面URL
func (q *QwenClient) GetChatURL() string {
	return qwenChatURL
}

// NavigateToHome 导航到千问首页（用于加载cookies后使cookies生效）
func (q *QwenClient) NavigateToHome() error {
	ctx := q.session.Ctx

	log.Printf("正在导航到千问首页: %s", qwenChatURL)

	// 导航到主页
	if err := chromedp.Run(ctx,
		chromedp.Navigate(qwenChatURL),
	); err != nil {
	...
	// 注入反检测脚本
	log.Println("注入反检测脚本...")
	if err := q.session.InjectAntiDetection(); err != nil {
...
// CheckLoggedIn 检查是否已登录（通过页面元素判断）
// 千问登录后通常有用户头像，检查相关选择器
func (q *QwenClient) CheckLoggedIn() bool {
	ctx := q.session.Ctx
	...
	err = chromedp.Run(ctx,
		chromedp.Evaluate(`
			(function() {
				// 检查是否有用户头像（千问登录后通常有头像元素）
				var avatar = document.querySelector('.avatar, [class*="avatar"], [class*="user-avatar"], [class*="UserAvatar"]');
				if (avatar) return true;

				// 检查是否有用户菜单或用户名（已登录特征）
				var userMenu = document.querySelector('[class*="user"], [class*="profile"], [class*="login-success"]');
				if (userMenu) return true;

				// 检查是否有聊天输入框（已登录才能看到输入框）
...
func (q *QwenClient) OpenLoginPage() error {
	ctx := q.session.Ctx

	log.Println("正在打开千问网站...")
	log.Printf("Session Context: %v", ctx)

	// 导航到千问登录页面
	log.Printf("开始导航到: %s", qwenLoginURL)
	if err := chromedp.Run(ctx,
		chromedp.Navigate(qwenLoginURL),
	); err != nil {
		log.Printf("导航失败: %v", err)
		return fmt.Errorf("导航失败: %v", err)
	}
...
// Ask 向千问提问（智能获取AI回复）
// 创建新对话并发送问题，返回完整答案
func (q *QwenClient) Ask(question string) (*platform.AskResult, error) {
	ctx, cancel := context.WithTimeout(q.session.Ctx, 90*time.Second)
	defer cancel()
	...
	if newChatResult == "no button" || newChatResult == "" {
		...
		chromedp.Run(ctx, chromedp.Navigate(qwenChatURL))
		chromedp.Run(ctx, chromedp.Sleep(2*time.Second))
	} else {
		chromedp.Run(ctx, chromedp.Sleep(2*time.Second))
	}
	...
	// 尝试点击发送按钮
	if err := q.sendQuestion(ctx, question); err != nil {
	...
	anomalyType, keyword, anomalyContext := q.detectPageAnomaly(ctx)
	...
	if anomalyType == "captcha" {
		...
		// 尝试自动处理验证
		passed, errMsg := q.handleCaptcha(ctx)
		if !passed {
			close(streamChan)
			result.IsBot = true
	...
	for time.Since(startTime) < maxWaitTime {
		...
		// 每轮循环先检查页面异常状态
		loopAnomalyType, _, _ := q.detectPageAnomaly(ctx)
	...
	answer, thinking := q.extractAIReply(ctx)
...
func (q *QwenClient) AskInConversation(question string) (*platform.AskResult, error) {
	ctx, cancel := context.WithTimeout(q.session.Ctx, 90*time.Second)
	defer cancel()
	...
	// 检查页面异常提示
	anomalyType, _, anomalyContext := q.detectPageAnomaly(ctx)
	if anomalyType == "input_error" {
		close(streamChan)
		result.IsBot = false
		result.DetectInfo = fmt.Sprintf("输入超限: %s", anomalyContext)
	...
	// 等待AI回复完成
	answer, thinking := q.waitForAIResponse(ctx)
...
func (q *QwenClient) StartNewConversation(initialMessage string) (*platform.AskResult, error) {
	ctx, cancel := context.WithTimeout(q.session.Ctx, 90*time.Second)
	defer cancel()
	...
	if err := q.sendQuestion(ctx, initialMessage); err != nil {
	...
	if anomalyType == "captcha" {
		...
		chromedp.Run(ctx, chromedp.Sleep(500*time.Millisecond))
		q.sendQuestion(ctx, initialMessage)
		chromedp.Run(ctx, chromedp.Sleep(2*time.Second))
	}
...
func (q *QwenClient) sendQuestion(ctx context.Context, question string) error {
	// 尝试点击发送按钮
	...
	if !sendClicked {
		...
		err := chromedp.Run(ctx, chromedp.Focus("textarea", chromedp.ByQuery))
		if err != nil {
...
func (q *QwenClient) waitForAIResponse(ctx context.Context) (string, string) {
	maxWaitTime := 90 * time.Second
	checkInterval := 2 * time.Second
	startTime := time.Now()
	...
	// 提取回复内容
	return q.extractAIReply(ctx)
}

// extractAIReply 从页面提取AI回复，分离思考过程和正式回复
func (q *QwenClient) extractAIReply(ctx context.Context) (string, string) {
	var jsResult string
	var thinking string
	var answer string
	chromedp.Run(ctx, chromedp.Evaluate(`
		(function() {
			// 千问页面需要排除的无关文本关键词
			var excludeKeywords = ['主题模式', '学习模式', '实名认证', '我的订单',
			                         '帮助与反馈', '注销账号', '关于我们', '退出登录',
			                         '积分', 'ICP备', '通义千问', 'Beijing',
			                         '扫描二维码', '新对话', '登录', '注册', '手机号', '验证码', '人机验证', 'captcha',
...
func (q *QwenClient) detectPageAnomaly(ctx context.Context) (string, string, string) {
	var resultStr string
	chromedp.Run(ctx, chromedp.Evaluate(`
		(function() {
			var errorKeywords = [
...
func (q *QwenClient) handleSliderCaptcha(ctx context.Context) bool {
	...
	// 2. 生成人类拖拽轨迹
	trajectory := q.generateHumanTrajectory(info.TrackWidth)

	// 3. 使用chromedp底层鼠标事件执行拖拽
	success := q.executeSliderDrag(ctx, info.SliderX, info.SliderY, trajectory)
	...
	if success {
		...
		// 检查验证是否通过
		anomalyType, _, _ := q.detectPageAnomaly(ctx)
		if anomalyType == "none" {
			log.Println("滑块验证通过！")
			return true
		}
...
func (q *QwenClient) generateHumanTrajectory(totalDistance float64) []struct {
	dx    float64
	dy    float64
	delay time.Duration
} {
...
func (q *QwenClient) executeSliderDrag(ctx context.Context, startX, startY float64, trajectory []struct {
	dx    float64
	dy    float64
	delay time.Duration
}) bool {
...
// 返回: 是否成功通过验证, 错误信息
func (q *QwenClient) handleCaptcha(ctx context.Context) (bool, string) {
	log.Println("检测到人机验证页面，尝试自动处理...")

	// 策略0：尝试自动处理滑块验证码（最常见的验证类型）
	for attempt := 0; attempt < 3; attempt++ {
		sliderOK := q.handleSliderCaptcha(ctx)
		if sliderOK {
			return true, ""
		}
		if attempt < 2 {
...
func (q *QwenClient) GetCookies() (string, error) {
	var cookieParts []string
	err := chromedp.Run(q.session.Ctx,
	...
	if err != nil {
		...
		return q.getCookiesFromDocument()
	}
...
func (q *QwenClient) getCookiesFromDocument() (string, error) {
