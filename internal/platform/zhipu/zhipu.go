// Package zhipu 智谱清言平台适配器
// 实现PlatformClient接口，将原有browser.ZhipuClient的功能适配到统一平台架构
package zhipu
...
const (
	...
	zhipuChatURL  = "https://chatglm.cn/main/"
)

// ZhipuClient 智谱清言平台客户端
...
func NewZhipuClient(session *browser.BrowserSession) *ZhipuClient {
...
func init() {
	platform.GetRegistry().Register(
		...
		func(session *browser.BrowserSession) platform.PlatformClient {
			return NewZhipuClient(session)
		},
		&platform.PlatformConfig{
			Platform: models.PlatformZhipu,
			LoginURL: "https://chatglm.cn/",
			ChatURL:  "https://chatglm.cn/main/",
			Selectors: map[string]string{
				"input_box":       "textarea",
				"send_button":     "button[type='submit']",
				"response_area":   ".message-content, .markdown-body",
...
func (z *ZhipuClient) GetPlatformName() string {
	return "zhipu"
}

// GetLoginURL 获取平台登录页面URL
func (z *ZhipuClient) GetLoginURL() string {
	return zhipuLoginURL
}

// GetChatURL 获取平台聊天页面URL
func (z *ZhipuClient) GetChatURL() string {
	return zhipuChatURL
}
...
// 创建新对话并发送问题，返回完整答案
func (z *ZhipuClient) Ask(question string) (*platform.AskResult, error) {
	ctx, cancel := context.WithTimeout(z.session.Ctx, 90*time.Second)
	defer cancel()
	...
	if newChatResult == "no button" || newChatResult == "" {
		...
		chromedp.Run(ctx, chromedp.Navigate("https://chatglm.cn/main/alltoolsdetail?lang=zh"))
		chromedp.Run(ctx, chromedp.Sleep(2*time.Second))
	} else {
		chromedp.Run(ctx, chromedp.Sleep(2*time.Second))
	}
	...
	// 调试：打印问题信息（使用UTF-8安全的方式）
	qRunes := []rune(question)
	if len(qRunes) > 0 {
		debugCount := 10
		if debugCount > len(qRunes) {
			debugCount = len(qRunes)
		}
		log.Printf("原始问题rune数量: %d, 字节长度: %d", len(qRunes), len(question))
		log.Printf("原始问题前%d个rune的Unicode码点: %v", debugCount, func() []rune {
			if len(qRunes) < debugCount {
				return qRunes
			}
			return qRunes[:debugCount]
		}())
	}
	...
	anomalyType, keyword, anomalyContext := z.detectPageAnomaly(ctx)
	...
	if anomalyType == "captcha" {
		...
		// 尝试自动处理验证
		passed, errMsg := z.handleCaptcha(ctx)
		if !passed {
			close(streamChan)
			result.IsBot = true
	...
	for time.Since(startTime) < maxWaitTime {
		...
		loopAnomalyType, _, _ := z.detectPageAnomaly(ctx)
	...
	answer, thinking := z.extractAIReply(ctx)
...
func (z *ZhipuClient) AskInConversation(question string) (*platform.AskResult, error) {
	ctx, cancel := context.WithTimeout(z.session.Ctx, 90*time.Second)
	defer cancel()
	...
	// 检查页面异常提示
	anomalyType, _, anomalyContext := z.detectPageAnomaly(ctx)
	if anomalyType == "input_error" {
		close(streamChan)
		result.IsBot = false
		result.DetectInfo = fmt.Sprintf("输入超限: %s", anomalyContext)
	...
	// 等待AI回复完成
	answer, thinking := z.waitForAIResponse(ctx)
...
func (z *ZhipuClient) StartNewConversation(initialMessage string) (*platform.AskResult, error) {
	ctx, cancel := context.WithTimeout(z.session.Ctx, 90*time.Second)
	defer cancel()
...
func (z *ZhipuClient) sendQuestion(ctx context.Context) {
	// 尝试点击发送按钮
	sendSelectors := []string{
		"button[class*='Send']",
		"button[class*='send']",
...
func (z *ZhipuClient) waitForAIResponse(ctx context.Context) (string, string) {
	maxWaitTime := 90 * time.Second
	checkInterval := 2 * time.Second
	startTime := time.Now()
	...
	// 提取回复内容
	return z.extractAIReply(ctx)
}

// extractAIReply 从页面提取AI回复，分离思考过程和正式回复
func (z *ZhipuClient) extractAIReply(ctx context.Context) (string, string) {
	var jsResult string
	var thinking string
	var answer string
	chromedp.Run(ctx, chromedp.Evaluate(`
...
func (z *ZhipuClient) GetCookies() (string, error) {
	...
	if err != nil {
		...
		return z.getCookiesFromDocument()
	}
...
func (z *ZhipuClient) getCookiesFromDocument() (string, error) {
...
func (z *ZhipuClient) detectPageAnomaly(ctx context.Context) (string, string, string) {
	var resultStr string
	chromedp.Run(ctx, chromedp.Evaluate(`
		(function() {
			var errorKeywords = [
...
func (z *ZhipuClient) handleSliderCaptcha(ctx context.Context) bool {
	...
	// 2. 生成人类拖拽轨迹
	trajectory := z.generateHumanTrajectory(info.TrackWidth)

	// 3. 使用chromedp底层鼠标事件执行拖拽
	success := z.executeSliderDrag(ctx, info.SliderX, info.SliderY, trajectory)
	...
	if success {
		...
		// 检查验证是否通过
		anomalyType, _, _ := z.detectPageAnomaly(ctx)
		if anomalyType == "none" {
			log.Println("滑块验证通过！")
			return true
		}
...
func (z *ZhipuClient) generateHumanTrajectory(totalDistance float64) []struct {
	dx    float64
	dy    float64
	delay time.Duration
} {
...
func (z *ZhipuClient) executeSliderDrag(ctx context.Context, startX, startY float64, trajectory []struct {
	dx    float64
	dy    float64
	delay time.Duration
}) bool {
...
// 返回: 是否成功通过验证, 错误信息
func (z *ZhipuClient) handleCaptcha(ctx context.Context) (bool, string) {
	log.Println("检测到人机验证页面，尝试自动处理...")

	// 策略0：尝试自动处理滑块验证码（最常见的验证类型）
	for attempt := 0; attempt < 3; attempt++ {
		sliderOK := z.handleSliderCaptcha(ctx)
		if sliderOK {
			return true, ""
		}
		if attempt < 2 {
	...
	for time.Since(captchaStart) < maxCaptchaWait {
		...
		if anomalyType == "none" {
			...
			// 检查是否还在聊天页面，如果不在则导航回来
			var currentURL string
			chromedp.Run(ctx, chromedp.Location(&currentURL))
			if !strings.Contains(currentURL, "chatglm.cn") {
